...
parfor (jj = 1:height(demogs),18)
    try
        ...
        imgbold2 = [];
        if(demogs2{jj,1}== 1)
            [~,imgbold2] = getHIVBOLDS(char(demogs2{jj,2}));
        elseif(demogs2{jj,1}== 2)
            [~,imgbold2] = getADRCBOLD(char(demogs2{jj,2}));       
        end       
        if(min(size(imgbold2))>=tslength)
            imgbold2 = imgbold2(:,1:tslength);
            imgbold2 = scale(imgbold2, gmmask);
            mask = zeros([48,64,48]);
            imgbold2 = reshape(imgbold2,[48,64,48,tslength]);
            imgbold2 = clearMask(imgbold2, gmmask, tslength);
            id = rand();
            finish(strcat('/data/ances2/NoiseID/bold/',string(jj),'_',string(id),'_X_X','.mat'),single(round(imgbold2,4)));
            finish(strcat('/data/ances2/NoiseID/mask/',string(jj),'_',string(id),'_X_X','.mat'),single(mask));
        end
    catch ME
        ME
    end
