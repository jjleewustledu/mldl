...
parfor (jj = 1:height(demogs),18)
    try
        ...
        imgbold2 = [];
        if(demogs2{jj,1}== 1)
            [imgbold2,imgbold] = getHIVBOLDS(char(demogs2{jj,2}));%!!!!!!!!!!!!!!!!!!!!!
        elseif(demogs2{jj,1}== 2)
            [imgbold2,imgbold] = getADRCBOLD(char(demogs2{jj,2}));
        else
            img = -9999;
        end
        ...
        if(min(size(imgbold))>=tslength)
            ...
            id = rand();
            finish(strcat('/data/ances2/NoiseID/bold/',string(jj),'_',string(id),'_XXX_X','.mat'),single(round(imgbold,4)));
            finish(strcat('/data/ances2/NoiseID/mask/',string(jj),'_',string(id),'_XXX_X','.mat'),single(mask));
        end
    catch ME
        ME
    end
